/** Dynamic news loader: fetches blogs.json and renders #actualites. */
...
export async function loadNews() {
  ...
  try {
    ...
    items = (Array.isArray(all) ? all : []).filter(i => i.status === 'published');
    if (items.length === 0) throw new Error('No published articles');
  } catch (err) {
    console.warn('[News] blogs.json failed:', err);
